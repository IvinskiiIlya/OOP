...
public class Program
{
    public static void Main()
    {
        ...
        for (int i = 0; i < numOfObjects; i++)
        {
            ...
            Discipline discipline = Discipline.AddDiscipline(i + 1, subjects[i], descriptions[i], new List<string> { lecturerSurname }, new List<int>{i+1});
            Global.Disciplines.Add(discipline);
            Lecturer lecturer = Lecturer.AddLecturer(i + 1, lecturerSurname, lecturerName, lecturerPatronymic, random.Next(25, 61), titles[random.Next(titles.Length)], new List<string> { subjects[i] }, new List<int>{i+1});
